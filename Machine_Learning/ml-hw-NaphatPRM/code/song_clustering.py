...
def sk_learn_cluster(X, K):
    ...
    """
    # TODO:
    sklearnKmeans = sklearn.cluster.KMeans(n_clusters=K, random_state=0).fit(X)
    return sklearnKmeans.cluster_centers_, sklearnKmeans.labels_
...
def cluster_songs(music_data, max_iters=300):
    ...
    preprocessed_data = min_max_scale(music_data)
    # TODO: Visualize the raw data with visualize_songs_clusters
    visualize_songs_clusters(preprocessed_data)
    # TODO: Use elbow_point_plot() to determine the optimal K (number of clusters)
    inertia_list = []
    for i in range(10):
        print(i)
        cluster_centroid, idx_element = Kmeans(preprocessed_data, i+1, max_iters=30).run()
        inertia_list.append(Kmeans(preprocessed_data, i+1, max_iters=30).inertia(cluster_centroid, idx_element))
    elbow_point_plot(np.arange(10), np.array(inertia_list))
    # TODO: Run k-means with your chosen K
    K = 3
    centroids_sklearn, idx_sklearn = sk_learn_cluster(preprocessed_data, K)
    centroids, idx = Kmeans(preprocessed_data, K, max_iters=30).run()
    # TODO: Use the visualize_songs_clusters() function to plot clusters
    visualize_songs_clusters(preprocessed_data, centroids=centroids_sklearn, centroid_indices=idx_sklearn,
                             is_lib_kmean=True)
    # TODO: Visually compare the output from your Kmeans object to sk_learn_cluster() (print clusters and plot the
    #  song clusters)
    visualize_songs_clusters(preprocessed_data, centroids=centroids, centroid_indices=idx,
                             is_lib_kmean=False)
